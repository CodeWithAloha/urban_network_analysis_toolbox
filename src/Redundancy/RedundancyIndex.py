...
def _redundant_unique_segments(network, dist_quota):
  ...
  o_parent, o_distance = find_shortest_path(network, "O", max_dist=dist_quota)
  # find all shortest paths from "D" at a max distance of the quota
  d_parent, d_distance = find_shortest_path(network, "D", max_dist=dist_quota)
  ...
  for node_id in nodes_in_reach:
    for edge_id in network.Nodes[node_id].Edges:
      if edge_id not in valid_segments and edge_id not in invalid_segments:
        ...
        success = False
        # trial 1: O --- [S-E] --- D
        d_1 = o_distance[edge.Start] + edge.Length + d_distance[edge.End]
        if d_1 <= dist_quota:
          _validate_path(o_parent, edge.Start)
          _validate_path(d_parent, edge.End)
          success = True
        # trial 2: O --- [E-S] --- D
        d_2 = o_distance[edge.End] + edge.Length + d_distance[edge.Start]
        if d_2 <= dist_quota:
